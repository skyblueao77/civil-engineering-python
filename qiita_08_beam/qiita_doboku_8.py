...
# ## このファイルの読み方
#
# このファイルは、JupyterやZedなどで実行できる**セル形式**のPythonファイルです。
# `#%%`の行がセルの区切り、`#%% [markdown]`から次のセル区切りまでが説明用のMarkdownセルです。
#
# ### 初回実行の手順
#
# 1. プロジェクトのルート（`pyproject.toml`があるフォルダ）で、次を実行します。
#
#    ```bash
#    uv sync
#    ```
#
# 2. エディタで、プロジェクトの`.venv`をPythonインタープリター（カーネル）に選択します。
# 3. このファイルは**上から順番に**セルを実行してください。後半のセルは、前半で作成した`x`、`V`、`M`、`w`などを使用します。
#
# Pythonファイルとして一括実行する場合は、プロジェクトのルートで次を実行できます。
#
# ```bash
# uv run python qiita_08_beam/qiita_doboku_8.py
# ```
#
# `uv run`を使うことで、プロジェクトで管理している依存関係（NumPy、Matplotlibなど）が自動的に使われます。
# まずは各セルを実行し、表示される図と計算結果を確認しながら読み進めてください。
...
#%%
# 入力値（以降の計算では、力をN、長さをmで統一します）。
L = 10.0             # 梁の長さ [m]
q = 10_000.0         # 等分布荷重 [N/m]（10 kN/m）

# 等分布荷重の合力は q * L です。
# 左右対称な梁なので、支点反力はその半分ずつになります。
RA = q * L / 2
RB = q * L / 2
...
# 梁上の位置 x を、左端（0 m）から右端（L m）まで501点作ります。
# x は配列なので、梁全体の計算を一度に実行できます。
L = 10.0
q = 10_000.0
...
#%%
# 曲げモーメント M(x) を、梁上の全ての位置 x で計算します。
M = q * L * x / 2 - q * x**2 / 2

# 最大値は梁中央 x = L/2 で発生します。
M_max = q * L**2 / 8
...
# このセルが、このファイルの中心となる計算セルです。
# まず入力値を設定し、その後に反力・せん断力・曲げモーメント・たわみを計算します。
# 梁の条件
L = 10.0             # 梁の長さ [m]
...
# 計算結果を、せん断力・曲げモーメント・たわみの3段に分けて表示します。
# VとMはN単位からkN単位へ、wはm単位からmm単位へ変換して描画します。
